...
## Prediction
Use the deleted races for prediction


```{r}
# Choosing relevant variables
rodriguez_pred <- rodriguez_df %>%  dplyr::select(c( driver, lap_time, 
                                tyre, team, 
                                position,
                                rainfall,
                                previous_position, pitstop_duration,
...
# rodriguez_pred$position <- NA
levels(rodriguez_pred$driver) <- levels(season23$driver)
levels(rodriguez_pred)
# Predict
season23.fit = bn.fit(tested_season23.hc, data = season23)

ALO5.pred = predict(season23.fit, data= rodriguez_pred[1:5,], node = "position", method = "parents", prob = T)
ALO5.pred
```
