...
```{r}
ggplot() + geom_bar(aes(x = season_df$tyre))
```
...
season_df$pitstop_duration <- as.numeric(season_df$pitstop_duration)
# Make bins
season_df$pitstop_duration <- cut(season_df$pitstop_duration, breaks = c(min(season_df$pitstop_duration, na.rm = TRUE), quantile(season_df$pitstop_duration, probs = c(.2, .4, .6, .8),na.rm=TRUE),max(season_df$pitstop_duration, na.rm = TRUE)), ordered_result = TRUE, include.lowest = TRUE, labels = c("fastest_pit", ".2", ".4", ".6", ".8"))
# Quantiles need more meaningful names

ggplot(data=subset(season_df, !is.na(pitstop_duration)), aes(x=pitstop_duration)) +
  geom_bar()

# Fill NAs with 0,1 bins
levels(season_df$pitstop_duration) <- c("fastest_pit", ".2", ".4", ".6", ".8", "no_stop")
season_df <- season_df %>% replace_na(list(pitstop_duration = "no_stop")) #change to "NA"

```


```{r}
ggplot(season_df, aes(x=pitstop_duration)) +
  geom_bar()
```
...
season_df$tyre <- factor(season_df$tyre, ordered = TRUE, levels = c("SOFT", "MEDIUM", "HARD")) 
